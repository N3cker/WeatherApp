...
const responseFiveDays = async function getWeather() {
  ...
  const markup = day
    .map((item) => {
      ...
      let nameDay = dt.toLocaleDateString("en", {
        ...
        <img class="wheather-icon" src="http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png" alt="${item.weather[0].description}"></img>
...
const responseFiveDaysMore = async function getWeatherMore() {
    const response = await fetch(api_url);
    const data = await response.json();
    const list = data.list;
