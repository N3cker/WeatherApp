const btnFiveDays = document.querySelector('.5days');
const infoWeather = document.querySelector('.wheather-list');


const api_url = "http://api.openweathermap.org/data/2.5/forecast?q=warszawa&cnt=5&lang=pl&units=metric&appid=c58ab9d92883ad1e6f51fe201539b277";


const responseApi = async function getWeather() {
  const response = await fetch(api_url);
  const data = await response.json();
  const list = data.list;
  const markup = list
    .map((item) => `<li class="wheather-list-item"
...
btnFiveDays.addEventListener("click", responseApi);