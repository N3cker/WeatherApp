...
export async function getData() {
  ...
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?q=${getCity()}&appid=${API_KEY}&units=metric`
  );
  const data = await response.json();
  const { list } = data;
  const temperature = list.map(temp => {
    return temp.main.temp;
  });
  const humidity = list.map(hum => {
    return hum.main.humidity;
  });
  const windSpeed = list.map(ws => {
    return ws.wind.speed;
  });
  const pressure = list.map(atm => {
    return atm.main.pressure;
  });
  const dayTime = list.map(date => {
    return date.dt;
  });

  let convertedDay = dayTime
    .map(day => new Date(day * 1000))
    .map(day =>
...
async function loadChart() {
  ...
  const dataTemps = await getData().catch(error => {
    console.error(error);
